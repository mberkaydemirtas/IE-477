...
def _plot_day_gantt(
    ...
    if t0_local is not None and day_start <= t0_local <= day_end:
        ...
        ax.axvline(t0h, linestyle="--", linewidth=2.0)
        ax.text(t0h, len(resources) - 0.2, "t0", rotation=90, va="top", ha="right")
...
def main():
    ...
    for fn in os.listdir(outdir):
        if fn.startswith(f"scenario{sid}_reschedule_machine_") and fn.endswith(".png"):
            os.remove(os.path.join(outdir, fn))
        if fn.startswith(f"scenario{sid}_reschedule_station_") and fn.endswith(".png"):
            os.remove(os.path.join(outdir, fn))

    cal = _calendar_from_meta(res)
    schedule_local = _convert_schedule_to_local(res_sched, cal)
    ...
    try:
        if t0 is not None:
            t0_local = _business_hours_to_local_dt(float(t0), cal)
    except Exception:
        t0_local = None
    ...
    while day_cursor <= day_last:
        ...
        _plot_day_gantt(
            schedule_local,
            "machine",
            f"Scenario {sid} - Reschedule (Machine) - {date_text} {weekday_name}",
            os.path.join(outdir, f"scenario{sid}_reschedule_machine_d{date_text}.png"),
            day_start,
            day_end,
            shift_start=cal["shift_start"],
            workday_hours=cal["workday_hours"],
        ...
        _plot_day_gantt(
            schedule_local,
            "station",
            f"Scenario {sid} - Reschedule (Station) - {date_text} {weekday_name}",
            os.path.join(outdir, f"scenario{sid}_reschedule_station_d{date_text}.png"),
            day_start,
            day_end,
            shift_start=cal["shift_start"],
            workday_hours=cal["workday_hours"],
