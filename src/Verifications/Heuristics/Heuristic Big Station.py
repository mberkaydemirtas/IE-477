...
NOTE (DATA CHANGE):
This version is arranged to make the optimal solution tardy (T_max > 0)
by (1) increasing processing times and (2) tightening due dates.
"""
...
# ===============================
#  PROCESSING TIMES (CHANGED to force tardiness)
# ===============================
...
for i in I:
    for m in M_i[i]:
        base = 6 + (i % 7)            # 6..12 (was 3..7)
        machine_add = (m - 1) * 0.8   # bigger spread (was 0.5)
...
# ===============================
#  DUE DATES (CHANGED to force tardiness)
# ===============================

# Due dates d_j (TIGHT)
d_i = {}
due_base = 22.0  # was 30.0
Iend = [O_j[j][-1] for j in J]  # last op of each job
for idx, i_last in enumerate(Iend):
    d_i[i_last] = due_base + 1.0 * idx   # 22..29 (was 30..51)
...
# ===============================
#  GRINDING / PAINTING (kept as original)
# ===============================
...
model = Model("Welding_Full_LaTeX_Integrated_BigStationStress_TardyData")
